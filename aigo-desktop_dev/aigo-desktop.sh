...
# TODO: change user ubuntu to aigo
# - 建立 root 密碼
#   $ sudo passwd root 
# - $ exit , 登出 ubuntu 帳號
# - 登入 root 帳號
# - 更改 ubuntu 帳號名稱為 aigo 及 home 目錄為 /home/aigo
#   $ usermod -l aigo -d /home/aigo -m ubuntu -c 'AiGO'
# - 更改 ubuntu 群組名稱為 aigo
#   $ groupmod -n aigo ubuntu
# - 更改 aigo 帳號密碼
#   $ passwd aigo
# - 刪除 root 密碼
#   $ passwd -d root
...
RET=$?
if [ $RET -eq 1 ] ; then
  usermod -l $UNAME -d $UHOME -m ubuntu -c 'AiGO'
  groupmod -n $UNAME ubuntu
  passwd -d root

  sed -i "s/ubuntu/$UNAME/" /etc/sudoers.d/90-cloud-init-users
fi
...
# fix hostapd fail or dnsmasq dhcp fail
...
if [ $RET -eq 1 ] ; then
  ...
  # nm-tray
  #apt install lxqt-session lxqt-panel qterminal openbox obconf-qt nm-tray xserver-xorg-video-fbdev xserver-xorg-input-evdev -y 2>&1 | tee $LOG_DIR/apt-install_lxqt.log
fi
...
# nm-tray
#U2004_PURGE_PKGS+=' netplan.io network-manager-gnome'
...
# no plugin hdmi cable - fixed hdmi mode & framebuffer_width/height 1280x720
grep 'AiGO modify' /boot/firmware/usercfg.txt
RET=$?
if [ $RET -eq 1 ] ; then
  echo '
#***** AiGO modify *****#
[pi4]
# armhf default is unset
dtoverlay=vc4-fkms-v3d

[pi3]
# HDMI output group to be either DMT
...
# for all 4 USB ports combined from 0.6A to 1.2A.
max_usb_current=1
...
# remove systemd-resolved link /etc/resolv.conf -> ../run/systemd/resolve/stub-resolv.conf
...
if [ -f /etc/iptables/rules.v4 ] ; then
  # except if their names end in ".dpkg-dist",".dpkg-old" or ".dpkg-new"
  cp -a /etc/iptables/rules.v4 /etc/rules.v4.dpkg-dist
  cat << EOF >> /etc/iptables/rules.v4
# Generated by iptables-save v1.6.0 on Mon Nov 21 20:39:16 2016
*nat
...
-A POSTROUTING -o uap0 -j MASQUERADE
COMMIT
# Completed on Mon Nov 21 20:39:16 2016
# Generated by iptables-save v1.6.0 on Mon Nov 21 20:39:16 2016
*filter
:INPUT ACCEPT [5:396]
:FORWARD ACCEPT [0:0]
...
RET=0
if [ $RET -eq 1 ] ; then
  # change RaspAP wlan0 IP
  # except if their names end in ".dpkg-dist",".dpkg-old" or ".dpkg-new"
  cp -a /etc/dhcpcd.conf /etc/dhcpcd.conf.dpkg-dist
  sed -i 's/ip_address=10.3.141.1/ip_address=192.168.11.254/g' /etc/dhcpcd.conf
  sed -i 's/routers=10.3.141.1/routers=192.168.11.254/g' /etc/dhcpcd.conf

# change RaspAP wlan0 DHCP IP & IP range
# except if their names end in ".dpkg-dist",".dpkg-old" or ".dpkg-new"
  cp -a /etc/dnsmasq.d/090_wlan0.conf /etc/dnsmasq.d/090_wlan0.conf.dpkg-dist
  sed -i 's/dhcp-range=10.3.141.50,10.3.141.255/dhcp-range=192.168.11.180,192.168.11.189/g' /etc/dnsmasq.d/090_wlan0.conf

# disable ipv6
#grep 'ipv6.disable=1' /boot/firmware/cmdline.txt
#RET=$?
#if [ $RET -eq 1 ] ; then
#  sed -i 's| fixrtc| fixrtc ipv6.disable=1|' /boot/firmware/cmdline.txt
#fi
fi
...
RET=$?
if [ $RET -eq 1 ] ; then
  apt install bind9-dnsutils -y 2>&1 | tee $LOG_DIR/apt-install_bind9-dnsutils.log
fi
